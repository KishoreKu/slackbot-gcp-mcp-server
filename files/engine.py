...
class SignalResult:
    ...
    label:       str
    triggered:   bool
    score_delta: float          # contribution to composite score (0–1)
    severity:    str = "low"    # low | medium | high
    detail:      str = ""       # internal detail for prompt construction


# ── Baseline shape (what Firestore returns) ──────────────────────────────────
#
# {
...
#   "known_merchants":          list[str],   # merchant names seen 3+ times
#   "known_countries":          list[str],   # ISO 3166 codes
#   "known_devices":            list[str],   # device_id list
#   "known_merchant_categories":list[str],
#   "typical_hours":            list[int],   # hours 0-23 with activity
#   "typical_days":             list[int],   # weekdays 0=Mon
#   "avg_daily_spend":          float,
#   "spend_today":              float,
#   "spend_this_week":          float,
#   "last_transaction_at":      str | None,  # ISO timestamp
#   "high_risk_flags":          int,         # prior high-risk tx count
# }
...
HIGH_RISK_CATEGORIES = {
    "CRYPTO", "GAMBLING", "WIRE", "FOREX", "MONEY_SERVICE",
    "CASH_ADVANCE", "PAWN", "ADULT", "MARIJUANA",
}

MEDIUM_RISK_CATEGORIES = {
    "ACH_TRANSFER", "P2P", "GIFT_CARD", "PRECIOUS_METALS",
    "INTERNATIONAL_WIRE",
}
...
def signal_amount_velocity(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Transaction is significantly larger than the user's historical average."""
    avg = baseline.get("avg_transaction_amount", 0)
    if avg <= 0:
        return SignalResult("amount_velocity_spike", "Amount velocity spike",
                            False, 0.0, detail="no baseline")

    ratio = tx.amount / avg
    if ratio >= 10:
        return SignalResult("amount_velocity_spike", "Amount velocity spike",
                            True, 0.55, severity="high",
                            detail=f"{ratio:.1f}× user's avg (${avg:.2f})")
    if ratio >= 5:
        return SignalResult("amount_velocity_spike", "Amount velocity spike",
                            True, 0.35, severity="medium",
                            detail=f"{ratio:.1f}× user's avg (${avg:.2f})")
    if ratio >= 3:
        return SignalResult("amount_velocity_spike", "Amount velocity spike",
                            True, 0.15, severity="low",
                            detail=f"{ratio:.1f}× user's avg (${avg:.2f})")
    return SignalResult("amount_velocity_spike", "Amount velocity spike",
                        False, 0.0)


def signal_new_merchant(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Merchant has never been seen for this user."""
    known = set(baseline.get("known_merchants", []))
    if not known:
        return SignalResult("new_merchant", "New merchant",
                            False, 0.0, detail="no baseline")
    name = tx.merchant.name.lower().strip()
    is_new = not any(name in k.lower() for k in known)
    if is_new:
        return SignalResult("new_merchant", "New merchant",
                            True, 0.12, severity="low",
...
def signal_new_merchant_category(
    ...
    """Transaction category not previously seen for this user."""
    ...
    if known_cats and cat not in known_cats:
        return SignalResult("new_merchant_category", "New merchant category",
                            True, 0.10, severity="low",
                            detail=f"Category '{cat}' first seen for this user")
    return SignalResult("new_merchant_category", "New merchant category", False, 0.0)


def signal_high_risk_category(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Merchant falls in a high-risk MCC category."""
    cat = tx.merchant.category.upper()
    if cat in HIGH_RISK_CATEGORIES:
        return SignalResult("high_risk_category", "High-risk merchant category",
                            True, 0.25, severity="high",
                            detail=f"Category '{cat}' is high-risk")
    if cat in MEDIUM_RISK_CATEGORIES:
        return SignalResult("high_risk_category", "High-risk merchant category",
                            True, 0.12, severity="medium",
                            detail=f"Category '{cat}' is medium-risk")
    return SignalResult("high_risk_category", "High-risk merchant category", False, 0.0)


def signal_geo_mismatch(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Device IP country doesn't match card's issuing country."""
    ...
    if card_country and device_country and card_country != device_country:
        return SignalResult("geo_mismatch", "Geo mismatch",
                            True, 0.30, severity="high",
                            detail=f"Card country {card_country} ≠ IP country {device_country}")
    return SignalResult("geo_mismatch", "Geo mismatch", False, 0.0)


def signal_new_country(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Transaction originates from a country not previously seen."""
    ...
    if known_countries and country and country not in known_countries:
        return SignalResult("new_country", "New country",
                            True, 0.20, severity="medium",
...
def signal_vpn_tor(
    ...
    """Device is using a VPN, Tor exit node, or anonymous proxy."""
    if not tx.device:
    ...
    if is_anon:
        ...
        return SignalResult("vpn_tor_proxy", "VPN/Tor/Proxy",
                            True, 0.25, severity="medium",
                            detail=f"Device is using {label}")
    return SignalResult("vpn_tor_proxy", "VPN/Tor/Proxy", False, 0.0)


def signal_new_device(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Transaction comes from a device ID not seen before for this user."""
    ...
    if known_devices and tx.device.device_id not in known_devices:
        return SignalResult("new_device", "New device",
                            True, 0.18, severity="medium",
                            detail=f"Device '{tx.device.device_id}' not in {len(known_devices)} known devices")
    return SignalResult("new_device", "New device", False, 0.0)


def signal_unusual_hour(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Transaction time is outside the user's normal activity hours."""
    ...
    if hour not in typical_hours:
        return SignalResult("unusual_hour", "Unusual hour",
                            True, 0.08, severity="low",
                            detail=f"Transaction at {hour:02d}:00 outside typical activity window")
    return SignalResult("unusual_hour", "Unusual hour", False, 0.0)


def signal_daily_spend_limit(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """User's spend today already exceeds their 90-day daily average."""
    ...
    if ratio >= 5:
        return SignalResult("daily_spend_exceeded", "Daily spend spike",
                            True, 0.30, severity="high",
                            detail=f"Projected daily spend ${projected:.2f} is {ratio:.1f}× avg (${avg_daily:.2f})")
    if ratio >= 3:
        return SignalResult("daily_spend_exceeded", "Daily spend spike",
                            True, 0.15, severity="medium",
                            detail=f"Projected daily spend ${projected:.2f} is {ratio:.1f}× avg (${avg_daily:.2f})")
    return SignalResult("daily_spend_exceeded", "Daily spend spike", False, 0.0)


def signal_rapid_succession(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Many transactions in last 24h — possible card testing."""
    count_24h = baseline.get("transaction_count_24h", 0)
    avg_30d_daily = baseline.get("transaction_count_30d", 0) / 30 if baseline.get("transaction_count_30d") else 0
    if avg_30d_daily <= 0:
        return SignalResult("rapid_succession", "Rapid succession", False, 0.0)
    ratio = (count_24h + 1) / (avg_30d_daily + 1)
    if ratio >= 4:
        return SignalResult("rapid_succession", "Rapid succession",
                            True, 0.35, severity="high",
...
def signal_new_user(
    ...
    """User has very few prior transactions — thin history."""
    count = baseline.get("transaction_count_90d", 0)
    if count < 3:
        return SignalResult("new_user", "New user — thin history",
                            True, 0.15, severity="medium",
                            detail=f"Only {count} transactions in 90 days")
    return SignalResult("new_user", "New user — thin history", False, 0.0)


def signal_prior_high_risk(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """User has previous transactions that were flagged high-risk."""
    flags = baseline.get("high_risk_flags", 0)
    if flags >= 3:
        return SignalResult("prior_high_risk", "Prior high-risk history",
                            True, 0.25, severity="high",
...
def signal_prepaid_card(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Transaction uses a prepaid card — higher fraud correlation."""
    if tx.card and tx.card.is_prepaid:
        return SignalResult("prepaid_card", "Prepaid card",
                            True, 0.12, severity="low",
                            detail="Prepaid cards have higher fraud correlation")
    return SignalResult("prepaid_card", "Prepaid card", False, 0.0)


def signal_large_round_amount(
    tx: TransactionRequest,
    baseline: dict,
) -> SignalResult:
    """Very large round-number amounts (e.g. $5,000.00) are atypical."""
    amount = tx.amount
    is_round = (amount % 100 == 0) and amount >= 1000
    if is_round:
        return SignalResult("large_round_amount", "Large round amount",
                            True, 0.08, severity="low",
...
# ── Signal registry ──────────────────────────────────────────────────────────
# Order matters for prompt construction (higher weight signals listed first)

SIGNAL_REGISTRY: list[tuple[str, Callable, float]] = [
    # (name, fn, weight_multiplier)
...
def run_all_signals(
    ...
    """Run every signal and return all results (triggered and not)."""
    results = []
...
def compute_composite_score(signal_results: list[SignalResult]) -> tuple[float, float]:
    """
    Composite score = weighted sum of triggered signal deltas, sigmoid-scaled.

    Returns (risk_score, confidence) both in [0, 1].
    Confidence is higher when multiple independent signals agree.
    """
    triggered = [s for s in signal_results if s.triggered]
    if not triggered:
        return 0.05, 0.95   # baseline low risk

    raw_sum = sum(s.score_delta for s in triggered)

    # Sigmoid to keep score in (0, 1) and dampen extreme stacking
    risk_score = 1 / (1 + math.exp(-3.5 * (raw_sum - 0.5)))
    risk_score = round(min(max(risk_score, 0.01), 0.99), 4)

    # Confidence: more diverse triggered signals = higher confidence
